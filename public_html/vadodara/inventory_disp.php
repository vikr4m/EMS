...
<table border="1" style ="border-collapse : collapse;" width = 70% align="center">
	...
	$i=1;
	$stmt = "SELECT * from vadodara WHERE `program`= '$program' limit $page1,15 ";
	...
	if ($stmt_exec-> num_rows > 0) 
	{
		...
		{
			$va_available = $row['va_available'];
			?>
			<tr>
				<td align='center'><?php echo $i; $i++ ?></td>
				<td align="center"><?php echo $row['book_id']; ?></td>
				<td align="center"><?php echo $row['book_name']; ?></td>
				<td align="center"><?php echo $row['va_available']; ?></td>
				<td align="center"><?php echo $row['va_given']; ?></td>
				<td align="center">
	...
	echo "</table>";
		$stmt1 = "SELECT * from vadodara  WHERE `program`= '$program'  ";
...
<?php	
	...
	$i=1;
	$stmt = "SELECT * from vadodara  limit $page1,15 ";
	$stmt_exec = mysqli_query($conn, $stmt);
...
{
	$stmt1 = "SELECT * from vadodara where va_available <= 20 ";
...
//Change the subject.
$mail->Subject = 'Stock updation mail for Vadodara.';
//Change the content as per your wish.
$mail->Body = $body;
//For client not supporting HTML rendering.
$mail->AltBody = '<h1>Hello !!Vadodara Stock needs updation.</h1>';
