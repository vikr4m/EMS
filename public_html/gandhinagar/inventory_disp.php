...
<table border="1" style ="border-collapse : collapse;" width = 70% align="center">
	...
	$i=1;
	$stmt = "SELECT * from gandhinagar WHERE `program`= '$program' limit $page1,15 ";
	...
	if ($stmt_exec-> num_rows > 0) 
	{
		while ($row = $stmt_exec->fetch_assoc()) 
		{
			...
			<tr>
				...
				<td align="center"><?php echo $row['ga_available']; ?></td>
				<td align="center"><?php echo $row['ga_given']; ?></td>	
				<td align="center">
	...
	echo "</table>";
		$stmt1 = "SELECT * from gandhinagar  WHERE `program`= '$program'  ";
...
<?php	
	...
	$i=1;
	$stmt = "SELECT * from gandhinagar  limit $page1,15 ";
	$stmt_exec = mysqli_query($conn, $stmt);
...
{
	$stmt1 = "SELECT * from gandhinagar where ga_available <= 20 ";
...
//Change the subject.
$mail->Subject = 'Stock updation mail for Gandhinagar.';
//Change the content as per your wish.
$mail->Body = $body;
//For client not supporting HTML rendering.
$mail->AltBody = '<h1>Hello !!Gandhinagar Stock needs updation.</h1>';
