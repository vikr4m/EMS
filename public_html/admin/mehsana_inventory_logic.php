...
function add_category() 
{
       ...
       $program =$_POST['program'];
        ...
        $addcategory= " SELECT * FROM mehsana WHERE `program`='$program' ";
      ////var_dump($sqlquery);  
 ...
 if(isset($_POST['add']))
{
    ...
    $book_name=$_POST['book_name'];
            ...
            $available_book = "SELECT me_available FROM mehsana  WHERE book_name = '$book_name'";
          ...
          if ($check_available_book-> num_rows > 0) 
              {
                while ($row = $check_available_book->fetch_assoc()) 
                {
                  ...
                  $ah_available = $row['me_available'];
                  //var_dump($row['ah_available']);

                  if($row['me_available'] <= 0 )
                  {
                  //var_dump($row['ah_available']);
            ...
            $book_query = "SELECT sid FROM me_book WHERE sid='$sid'AND book_name = '$book_name'";
            //var_dump($book_query);
            ...
            if($count_book > 0)
              ...
              {  
                $insert ="INSERT INTO me_book (sid,book_name) VALUES ('$sid','$book_name') ";
            
                if ($result = $conn->query($insert)) 
                {
                $minus= "UPDATE mehsana SET me_available = me_available -1  where book_name='$book_name' ";
                $quer = mysqli_query($conn,$minus);
             // var_dump($quer);
                $plus= "UPDATE mehsana SET me_given = me_given +1  where book_name='$book_name'";
...
function book_table() 
{
      ...
      $select_query="SELECT * FROM me_book where sid='$sid' ";
