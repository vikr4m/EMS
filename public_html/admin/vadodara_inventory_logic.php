...
function add_category() 
{
       ...
       $program =$_POST['program'];
        ...
        $addcategory= " SELECT * FROM vadodara WHERE `program`='$program' ";
      ////var_dump($sqlquery);  
 ...
 if(isset($_POST['add']))
{
    ...
    $book_name=$_POST['book_name'];
            ...
            $available_book = "SELECT va_available FROM vadodara  WHERE book_name = '$book_name'";
          ...
          if ($check_available_book-> num_rows > 0) 
            ...
            $book_query = "SELECT sid FROM va_book WHERE sid='$sid'AND book_name = '$book_name'";
            //var_dump($book_query);
            ...
            if($count_book > 0)
              ...
              {  
                $insert ="INSERT INTO va_book (sid,book_name) VALUES ('$sid','$book_name') ";
            
                if ($result = $conn->query($insert)) 
                {
                $minus= "UPDATE vadodara SET va_available = va_available -1  where book_name='$book_name' ";
                $quer = mysqli_query($conn,$minus);
             // var_dump($quer);
                $plus= "UPDATE vadodara SET va_given = va_given +1  where book_name='$book_name'";
...
function book_table() 
{
      ...
      $select_query="SELECT * FROM va_book where sid='$sid' ";
