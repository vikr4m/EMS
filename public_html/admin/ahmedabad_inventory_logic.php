...
function add_category() 
{
       ...
       $program =$_POST['program'];
        ...
        $addcategory= " SELECT * FROM ahmedabad WHERE `program`='$program' ";
      ////var_dump($sqlquery);  
 ...
 if(isset($_POST['add']))
{
    ...
    $book_name=$_POST['book_name'];
            ...
            $available_book = "SELECT ah_available FROM ahmedabad  WHERE book_name = '$book_name'";
          ...
          if ($check_available_book-> num_rows > 0) 
            ...
            $book_query = "SELECT sid FROM ah_book WHERE sid='$sid'AND book_name = '$book_name'";
            //var_dump($book_query);
            ...
            if($count_book > 0)
              ...
              {  
                $insert ="INSERT INTO ah_book (sid,book_name) VALUES ('$sid','$book_name') ";
            
                if ($result = $conn->query($insert)) 
                {
                $minus= "UPDATE ahmedabad SET ah_available = ah_available -1  where book_name='$book_name' ";
                $quer = mysqli_query($conn,$minus);
             // var_dump($quer);
                $plus= "UPDATE ahmedabad SET ah_given = ah_given +1  where book_name='$book_name'";
...
function book_table() 
{
      ...
      $select_query="SELECT * FROM ah_book where sid='$sid' ";
