...
<table border="1" style ="border-collapse : collapse;" width = 70% align="center">
	...
	$i=1;
	$stmt = "SELECT * from ahmedabad WHERE `program`= '$program' limit $page1,15 ";
	...
	if ($stmt_exec-> num_rows > 0) 
	{
		...
		{
			$ah_available = $row['ah_available'];
			?>
			<tr>
				<td align='center'><?php echo $i; $i++ ?></td>
				<td align="center"><?php echo $row['book_id']; ?></td>
				<td align="center"><?php echo $row['book_name']; ?></td>
				<td align="center"><?php echo $row['ah_available']; ?></td>
				<td align="center"><?php echo $row['ah_given']; ?></td>	
				<td align="center">

					<input type="button"   onclick="location.href = 'inventory_update.php?book_id=<?php echo $row['book_id'] ?>'"  value="Change Availability"/>
	...
	echo "</table>";
		$stmt1 = "SELECT * from ahmedabad  WHERE `program`= '$program'  ";
...
echo "<p align='center'>Pages:";
	...
	for($b=1; $b<=$a; $b++) 
	{
	?><a href="inventory_disp.php?page=<?php echo $b;?>&&program=<?php echo $program;?>" style="text-decoration: none"><?php echo $b." ";?></a> <?php
	}
	?>
</p>
<br><br>

<input type="button"   onclick="location.href = 'export_inventory.php?name=<?php echo $program;?>';"  value="Export"/>
<br><br><br><br>
<?php	
...
<?php	
	...
	$i=1;
	$stmt = "SELECT * from ahmedabad  limit $page1,15 ";
	$stmt_exec = mysqli_query($conn, $stmt);
...
echo "<p align='center'>Pages:";
	...
	for($b=1; $b<=$a; $b++) 
	{
	?><a href="inventory_disp.php?page=<?php echo $b;?>" style="text-decoration: none"><?php echo $b." ";?></a> <?php
...
<input type="button"   onclick="location.href = 'export_inventory.php?';"  value="Export"/>
<br><br><br><br>
<?php	
...
{
	$stmt1 = "SELECT * from ahmedabad where ah_available <= 20 ";
...
//Change the subject.
$mail->Subject = 'Stock updation mail for Ahmedabad.';
//Change the content as per your wish.
$mail->Body = $body;
//For client not supporting HTML rendering.
$mail->AltBody = '<h1>Hello !!Ahmedabad Stock needs updation.</h1>';
